# app.py -- Gradio, deployed to Hugging Face Spaces
...
def run_comparison(question_text):
    ...
    paragraphs = [
        {"id": t, "title": t, "text": " ".join(s)}
        for t, s in zip(example["context"]["title"], example["context"]["sentences"])
    ]
    chunks = chunk_paragraphs(paragraphs, chunk_size=300, overlap=50)
    retriever = Retriever(collection_name="demo")
    retriever.index(chunks)

    single = answer_single_shot(question_text, retriever, top_k=5)
    iterative = answer_iterative(question_text, retriever, top_k=5, max_hops=3)

    return (
        f"{single['answer']}\n\nSources: {single['retrieved_source_ids']}",
        f"{iterative['answer']}\n\nHops: {iterative['hops']}\nSources: {iterative['retrieved_source_ids']}",
        example["answer"],
    )

demo = gr.Interface(
    fn=run_comparison,
    inputs=gr.Dropdown(choices=list(question_choices.keys()), label="Pick a HotpotQA question"),
    outputs=[
